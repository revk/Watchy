...
acc_init (void)
{
   ...
   {                            // Not initialised
      ...
      i2c_write_block (0, sizeof (bma423_config_file), bma423_config_file);
      i2c_write (0x59, 0x01);   // INIT_CTRL
      usleep (150000);

      status = i2c_read (0x2A); // Should be 1 now...
      if (!status)
      ...
      i2c_write (0x56, 0x02);   // INT1_MAP - step_counter_out
...
acc_steps (void)
{
   ...
   i2c_master_start (txn);
   i2c_master_write_byte (txn, (ACCADDRESS << 1) | I2C_MASTER_READ, true);
   i2c_master_read (txn, data, sizeof (data), I2C_MASTER_LAST_NACK);
   i2c_master_stop (txn);
   if ((e = i2c_master_cmd_begin (I2CPORT, txn, 10 / portTICK_PERIOD_MS)))
      ESP_LOGE (TAG, "Read fail %d", e);
   i2c_cmd_link_delete (txn);
